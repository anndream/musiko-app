...
import app.musiko.MusicViewModel
import app.musiko.R
import app.musiko.adapters.QueueAdapter
import app.musiko.databinding.*
import app.musiko.extensions.*
import app.musiko.fragments.*
import app.musiko.goPreferences
import app.musiko.helpers.*
import app.musiko.models.Music
import app.musiko.player.EqualizerUtils
import app.musiko.player.MediaPlayerHolder
import app.musiko.player.MediaPlayerInterface
import app.musiko.player.PlayerService
...
class MainActivity : AppCompatActivity(), UIControlInterface {
    ...
    // Booleans
    private val sDetailsFragmentExpanded get() = supportFragmentManager.isFragment(MusikoConstants.DETAILS_FRAGMENT_TAG)
    private val sErrorFragmentExpanded get() = supportFragmentManager.isFragment(MusikoConstants.ERROR_FRAGMENT_TAG)
    private val sEqFragmentExpanded get() = supportFragmentManager.isFragment(MusikoConstants.EQ_FRAGMENT_TAG)
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        if (sAppearanceChanged) {
            ...
            outState.putBoolean(
                    MusikoConstants.RESTORE_SETTINGS_FRAGMENT,
                    true
            )
        }
    }
    ...
    override fun onRequestPermissionsResult(
        ...
        when (requestCode) {
            MusikoConstants.PERMISSION_REQUEST_READ_EXTERNAL_STORAGE -> {
                ...
                if ((grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED)) {
                    ...
                    notifyError(MusikoConstants.TAG_NO_PERMISSION)
                }
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sRestoreSettingsFragment =
                savedInstanceState?.getBoolean(MusikoConstants.RESTORE_SETTINGS_FRAGMENT)
    ...
    private fun notifyError(errorType: String) {
        ...
        supportFragmentManager.addFragment(
                ...
                MusikoConstants.ERROR_FRAGMENT_TAG,
                false
        )
    }
    ...
    private fun finishSetup(music: MutableList<Music>?) {
        ...
        if (!music.isNullOrEmpty()) {
            ...
            notifyError(MusikoConstants.TAG_NO_MUSIC)
        }
    }
    ...
    private fun initFragmentAtPosition(fragmentIndex: Int) {
        when (fragmentIndex) {
            0 -> if (mArtistsFragment == null) {
                mArtistsFragment =
                        MusicContainersListFragment.newInstance(MusikoConstants.ARTIST_VIEW)
            }
            1 -> if (mAlbumsFragment == null) {
                mAlbumsFragment =
                        MusicContainersListFragment.newInstance(MusikoConstants.ALBUM_VIEW)
            }
            ...
            3 -> if (mFoldersFragment == null) {
                mFoldersFragment =
                        MusicContainersListFragment.newInstance(MusikoConstants.FOLDER_VIEW)
            }
    ...
    private fun openDetailsFragment(
        ...
        if (!sDetailsFragmentExpanded) {
            ...
            supportFragmentManager.addFragment(mDetailsFragment, MusikoConstants.DETAILS_FRAGMENT_TAG, sEqFragmentExpanded)
        }
    }
        ...
        mNowPlayingBinding.npSeekBar.setOnSeekBarChangeListener(
                object : SeekBar.OnSeekBarChangeListener {
                    ...
                    override fun onStopTrackingTouch(seekBar: SeekBar?) {
                        ...
                        if (mMediaPlayerHolder.state != MusikoConstants.PLAYING) {
    ...
    private fun saveSongToPref() {
        if (::mMediaPlayerHolder.isInitialized && !mMediaPlayerHolder.isPlaying || mMediaPlayerHolder.state == MusikoConstants.PAUSED) mMediaPlayerHolder.apply {
    ...
    private fun updatePlayingStatus(isNowPlaying: Boolean) {
        val isPlaying = mMediaPlayerHolder.state != MusikoConstants.PAUSED
        val drawable =
                if (isPlaying) {
                    R.drawable.ic_pause
                } else {
            ...
            mMediaPlayerHolder.apply {
                ...
                if (isMediaPlayer && mMediaPlayerHolder.isPlaying) {
                    ...
                    if (!songs.isNullOrEmpty()) {
                        ...
                        notifyError(MusikoConstants.TAG_SD_NOT_READY)
                    }
                }
    ...
    private fun getSongSource(selectedSong: Music?, launchedBy: String): String? {
        return when (launchedBy) {
            MusikoConstants.FOLDER_VIEW -> selectedSong?.relativePath
            MusikoConstants.ARTIST_VIEW -> selectedSong?.artist
            ...
            if (!EqualizerUtils.hasEqualizer(this)) {
                if (!sEqFragmentExpanded) {
                    ...
                    synchronized(closeNowPlayingDialog()) {
                        ...
                        supportFragmentManager.addFragment(mEqualizerFragment, MusikoConstants.EQ_FRAGMENT_TAG, sDetailsFragmentExpanded)
                    }
                }
            } else {
    ...
    private fun handleIntent(intent: Intent) {
        ...
        intent.data?.let { returnUri ->
            ...
            try {
                ...
                onSongSelected(song, albumSongs, MusikoConstants.ARTIST_VIEW)

            } catch (e: Exception) {
                e.printStackTrace()
                notifyError(MusikoConstants.TAG_NO_MUSIC_INTENT)
            }
        }
    }
    ...
    private val mMediaPlayerInterface = object : MediaPlayerInterface {
        ...
        override fun onStateChanged() {
            ...
            if (mMediaPlayerHolder.state != MusikoConstants.RESUMED && mMediaPlayerHolder.state != MusikoConstants.PAUSED) {
                updatePlayingInfo(false)
